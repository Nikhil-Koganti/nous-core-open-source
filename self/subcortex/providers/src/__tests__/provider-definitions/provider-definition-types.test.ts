...
type _ProviderVendorKeyIsExact = Expect<
  Equal<ProviderVendorKey, 'anthropic' | 'codex-cli' | 'openai' | 'ollama' | 'qwen-code'>
>;
type _BootstrapProviderKeyIsExact = Expect<
  Equal<BootstrapProviderKey, 'anthropic' | 'codex-cli' | 'openai' | 'ollama' | 'qwen-code'>
>;
...
describe('provider definition type derivation', () => {
  it('derives runtime roster keys from the production tuple', () => {
    ...
    expect(keys.sort()).toEqual(['anthropic', 'codex-cli', 'ollama', 'openai', 'qwen-code']);
  });
